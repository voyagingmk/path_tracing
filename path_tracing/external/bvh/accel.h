...
namespace accel
{
    ...
    public:
        ...
        f32& y() { return y_; };
        f32& z() { return z_; };
        Vector3 mult(const Vector3& b) const { return Vector3(x_ * b.x_, y_ * b.y_, z_ * b.z_); }
        Vector3& norm() { return *this = *this * (1 / sqrt(x_ * x_ + y_ * y_ + z_ * z_)); }
        double dot(const Vector3& b) const { return x_ * b.x_ + y_ * b.y_ + z_ * b.z_; } 
        Vector3 cross(Vector3& b) { return Vector3(y_ * b.z_ - z_ * b.y_, z_ * b.x_ - x_ * b.z_, x_ * b.y_ - y_ * b.x_); }
        Vector3 Min(Vector3& b) { return Vector3(std::min(x_, b.x_), std::min(y_, b.y_), std::min(z_, b.z_)); }
        Vector3 Max(Vector3& b) { return Vector3(std::max(x_, b.x_), std::max(y_, b.y_), std::max(z_, b.z_)); }
